...
*Present this table to the USER and ask for approval before writing the tests, or proceed if the user has requested autonomous testing.*
...
Only after adversarial tests are written. If overall coverage is below standard (e.g., 70%):
...
Run tests:
```bash
go test -v -cover ./... # or equivalent for the specific package
```
